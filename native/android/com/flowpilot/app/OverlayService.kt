...
class OverlayService : Service() {
    ...
    companion object {
        private const val OVERLAY_TYPE =
        ...
        Thread {
            while (true) {
                try {
                    Thread.sleep(1000)
                    runOnUiThread {
                        vel?.text = FlowBridge.kmAtual(this@OverlayService).toString() // trocar por velocidade real
                        eta?.text = FlowBridge.textoNotificacao(this@OverlayService)
                    }
    ...
    private fun runOnUiThread(bloco: () -> Unit) {
        android.os.Handler(mainLooper).post(bloco)
    }
